...
years.data <- data.all %>% dplyr::distinct(paper_id, published)
...
years.plot
ggsave("../../figs/years.png", plot=years.plot, width=4, height=3, units="in", dpi=300, scale =1.5)
```
...
taxa.plot <- ggplot(data=data.all,aes(sort_factor(species_type))) + 
  geom_bar(fill = pal[1]) + coord_flip() + ylab("Number of models") + xlab("Modelled taxa") + scale_x_discrete(labels = c("Cnidarian","Macroalgae","Bryozoa","Echinoderm","Gastropod","Polychaete","Mollusc","Coral","Crustacean","Bivalvia","Fish","Generic")) + theme_cowplot(font_size=12,font_family = "sans")
...
data.models <- dplyr::distinct(data.all,title, model_reuse, model_name, motivation, geographical_zone)
...
```{r}
data.models <- dplyr::distinct(data.all,title, physical_model, nested_submodels,model_resolution_min, model_integration, model_time_step)
...
papers.comparisons <- papers.comparisons %>% dplyr::distinct(paper_id,published,behaviours,implemented,species_type) %>% filter(implemented == TRUE) %>% mutate(is_fish = species_type == 'Fish') %>% mutate(is_invert = species_type != 'Fish' & species_type != 'Generic' & species_type != 'Macroalgae') %>%
...
sr.test <- t.test(self_recruitment_mean ~ system, data=data.all, var.equal=TRUE)
bartlett.test(local_retention_mean ~ system, data=data.all)
lr.test <- t.test(local_retention_mean ~ system, data=data.all, var.equal=TRUE)
bartlett.test(settlement_success_mean ~ system, data=data.all)
ss.test <- t.test(settlement_success_mean ~ system, data=data.all, var.equal=TRUE)
bartlett.test(distance_travelled_mean ~ system, data=data.all)
dd.test <- t.test(distance_travelled_mean ~ system, data=data.all, var.equal=FALSE)
p.values <- c(sr.test$p.value,lr.test$p.value,ss.test$p.value,dd.test$p.value)
print("sr lr ss dd")
p.values
print("adjusted")
p.adjust(p.values, method="fdr")
data.open<- filter(data.all, system == "Open")
data.closed<- filter(data.all, system == "Closed")
mean(data.open$local_retention_mean, na.rm = TRUE)
mean(data.closed$local_retention_mean, na.rm = TRUE)
```
...
data.physical.models.ss <- data.all %>% filter(settlement_success_mean  > 0) %>% group_by(physical_model) %>% filter(n() > 1) %>% filter(physical_model != 'NA')
data.physical.models.sr <- data.all %>% filter(self_recruitment_mean  > 0) %>% group_by(physical_model) %>% filter(n() > 1) %>% filter(physical_model != 'NA')
data.physical.models.lr <- data.all %>% filter(local_retention_mean  > 0) %>% group_by(physical_model) %>% filter(n() > 1)%>% filter(physical_model != 'NA')
data.physical.models.dd <- data.all %>% filter(distance_travelled_mean  > 0) %>% group_by(physical_model) %>% filter(n() > 1) %>% filter(physical_model != 'NA')
...
physical.plot1 <- ggplot(data.physical.models.lr,aes(physical_model,local_retention_mean)) + labs(y="Local retention (%)", x="Hydrodynamic model") + geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) + coord_flip() + theme_cowplot(font_size=10,font_family = "sans")
physical.plot2 <- ggplot(data.physical.models.sr,aes(physical_model,self_recruitment_mean)) + labs(y="Self-recruitment (%)", x="Hydrodynamic model") +geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) +coord_flip() + theme_cowplot(font_size=10,font_family = "sans")
physical.plot3 <- ggplot(data.physical.models,aes(physical_model,settlement_success_mean))+ labs(y="Settlement success (%)", x="Hydrodynamic model") + geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) +coord_flip() + theme_cowplot(font_size=10,font_family = "sans")
physical.plot4 <- ggplot(data.physical.models,aes(physical_model,distance_travelled_mean))+ labs(y="Dispersal distance (km)", x="Hydrodynamic model") + geom_boxplot() + coord_flip()+ theme_cowplot(font_size=10,font_family = "sans")
...
sr.test <- summary(resolution.sr_lm)
ss.test <- summary(resolution.ss_lm)
lr.test <- summary(resolution.lr_lm)
dd.test<- summary(resolution.dd_lm)
p.values <- c(sr.test$coefficients[2,4],lr.test$coefficients[2,4],ss.test$coefficients[2,4],dd.test$coefficients[2,4])
print("sr lr ss dd")
p.values
print("adjusted")
p.adjust(p.values, method="fdr")
...
resolution1 <- ggplot(resolution.data,aes(model_resolution_min,local_retention_mean)) + 
  ...
  annotate(geom = "text", x = 14, y = 0.95, label = c("a")) +
...
resolution2 <- ggplot(resolution.data,aes(model_resolution_min,self_recruitment_mean)) +   
  ...
  scale_colour_few() + 
  scale_y_continuous(limits = c(0, 1.0)) +
  annotate(geom = "text", x = 14, y = 0.95, label = c("b")) +
  theme(axis.title = element_text(size = 9), axis.text = element_text(size = 8))
resolution3 <- ggplot(resolution.data,aes(model_resolution_min,settlement_success_mean)) + 
  ...
  scale_colour_few() + 
  scale_y_continuous(limits = c(0, 1.0)) +
  annotate(geom = "text", x = 14, y = 0.95, label = c("c")) +
  theme(axis.title = element_text(size = 9), axis.text = element_text(size = 8))
resolution4 <- ggplot(resolution.data,aes(model_resolution_min,distance_travelled_mean)) + 
  ...
  theme_cowplot(font_size=12,font_family = "sans") + 
  scale_colour_few() + 
  annotate(geom = "text", x = 14, y = 900, label = c("d")) +
  theme(axis.title = element_text(size = 9), axis.text = element_text(size = 8))
...
sr.test <- summary(pld.sr_lm)
ss.test <- summary(pld.ss_lm)
lr.test <- summary(pld.lr_lm)
dd.test<- summary(pld.dd_lm)
p.values <- c(sr.test$coefficients[2,4],lr.test$coefficients[2,4],ss.test$coefficients[2,4],dd.test$coefficients[2,4])
print("sr lr ss dd")
p.values
print("adjusted")
...
sr.test <- summary(window.sr_lm)
ss.test <- summary(window.ss_lm)
lr.test <- summary(window.lr_lm)
dd.test<- summary(window.dd_lm)
p.values <- c(sr.test$coefficients[2,4],lr.test$coefficients[2,4],ss.test$coefficients[2,4],dd.test$coefficients[2,4])
print("sr lr ss dd")
p.values
print("adjusted")
...
sr.test <- t.test(self_recruitment_mean ~ mortality, data=data.all, var.equal=TRUE)
lr.test <- t.test(local_retention_mean ~ mortality, data=data.all, var.equal=FALSE)
ss.test <- t.test(settlement_success_mean ~ mortality, data=data.all, var.equal=TRUE)
dd.test <- t.test(distance_travelled_mean ~ mortality, data=data.all, var.equal=FALSE)
p.values <- c(sr.test$p.value,lr.test$p.value,ss.test$p.value,dd.test$p.value)
print("sr lr ss dd")
p.values
print("adjusted")
...
lsmeans.sr <- lsmeans(beta.sr, pairwise ~ behaviours, adjust="fdr")
summary(lsmeans.sr)
cld(lsmeans.sr, alpha=0.05, Letters=letters, adjust="fdr")
```
...
lsmeans.lr <- lsmeans(beta.lr, pairwise ~ behaviours, adjust="fdr")
summary(lsmeans.lr)
cld(lsmeans.lr, alpha=0.05, Letters=letters, adjust="fdr")
```
...
lsmeans.ss <- lsmeans(beta.ss, pairwise ~ behaviours, adjust="fdr")
summary(lsmeans.ss)
cld(lsmeans.ss, alpha=0.05, Letters=letters, adjust="fdr")
```
...
lsmeans.dist <- lsmeans(glm.dist, ~ behaviours, adjust="fdr")
summary(pairs(lsmeans(glm.dist, ~ behaviours)), type = "response")
cld(lsmeans.dist,alpha = 0.05, Letters = letters, adjust="fdr")
```
