...
years.plot <- ggplot(years.data,aes(published)) + geom_bar(fill=pal[1]) + labs(x="Year published", y="Number of studies") + theme_few()
...
taxa.plot <- ggplot(data=data.all,aes(sort_factor(species_type))) + 
  geom_bar(fill = pal[1]) + coord_flip() + ylab("Number of models") + xlab("Modelled taxa") + scale_x_discrete(labels = c("Cnidarian","Gastropod","Macroalgae","Echinoderm","Polychaete","Coral","Crustacean","Mollusc","Bivalvia","Fish","Generic")) + theme_few()
...
comparisons.plot <- ggplot(papers.comparisons, aes(published)) + geom_bar(aes(fill = behaviours)) +labs( x = "", y = "Number of models") +theme_few()+ theme(legend.title = element_blank(), legend.position = "top",legend.key.size = unit(5,'mm'),axis.text.x=element_text(angle=90)) +  scale_fill_manual(values=pal,breaks=c("passive_movement","movement","sensory_ability","directed_swimming"), labels = c("Passive", "Movement", "Settlement", "Orientation")) + facet_grid(taxa ~ .)+ annotate(geom = "text", x = 7, y = 14, label = c("a","b")) + guides(fill=guide_legend(nrow=2,byrow=TRUE)) 
...
plot1 <- ggplot(data.models.lr,aes(model_name,local_retention_mean))+ labs(y="Local retention (%)", x="") + geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) +coord_flip() +annotate(geom = "text", x = 7, y = 0.95, label = c("a")) + theme_few() + scale_fill_few()
plot2 <- ggplot(data.models.sr,aes(model_name,self_recruitment_mean)) + labs(y="Self-recruitment (%)", x="") +geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) +coord_flip() +annotate(geom = "text", x = 16.5, y = 0.95, label = c("b")) + theme_few() + scale_fill_few()
plot3 <- ggplot(data.models.ss,aes(model_name,settlement_success_mean))+ labs(y="Settlement success (%)", x="") + geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) +coord_flip() + annotate(geom = "text", x = 12.5, y = 0.95, label = c("c")) + theme_few() + scale_fill_few()
plot4 <- ggplot(data.models.dd,aes(model_name,distance_travelled_mean))+ labs(y="Dispersal distance (km)", x="") + geom_boxplot() + coord_flip() + annotate(geom = "text", x = 11.5, y = 900, label = c("d")) + theme_few() + scale_fill_few()
...
plot1 <- ggplot(data.models.lr,aes(physical_model,local_retention_mean)) + labs(y="Local retention (%)", x="") + geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) + coord_flip() + annotate(geom = "text", x = 7, y = 0.95, label = c("a")) + theme_few() + scale_fill_few()
plot2 <- ggplot(data.models.sr,aes(physical_model,self_recruitment_mean)) + labs(y="Self-recruitment (%)", x="") +geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) +coord_flip() + annotate(geom = "text", x = 13.5, y = 0.95, label = c("b")) + theme_few() + scale_fill_few()
plot3 <- ggplot(data.models.ss,aes(physical_model,settlement_success_mean))+ labs(y="Settlement success (%)", x="") + geom_boxplot() + scale_y_continuous(limits = c(0, 1.0)) +coord_flip()+ annotate(geom = "text", x = 9.5, y = 0.95, label = c("c")) + theme_few() + scale_fill_few()
plot4 <- ggplot(data.models.dd,aes(physical_model,distance_travelled_mean))+ labs(y="Dispersal distance (km)", x="") + geom_boxplot() + coord_flip()+ annotate(geom = "text", x = 10.5, y = 900, label = c("d")) + theme_few() + scale_fill_few()
...
mort4 <- ggplot(data.all, aes(mortality, distance_travelled_mean)) + geom_boxplot()+ #geom_jitter(width = 0.1) +
  ...
  labs(x="", y="Dispersal distance (km)")+ theme_few() + scale_fill_few()+ scale_x_discrete(labels=c("No mortality","Mortality"))
