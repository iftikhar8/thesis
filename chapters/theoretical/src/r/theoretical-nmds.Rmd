...
data.norm.orientate.surface$behaviour <- rep(c("ohs-surface"))
...
data.norm.orientate$behaviour <- rep(c("ohs"))
...
phase1.nmds.plot.norm <- ggplot(phase1.points.norm, aes(MDS1, MDS2, colour=behaviour, group=sites,fill=behaviour))  + theme_bw() + theme(legend.position = "right") + guides(fill=guide_legend(title.position = "top")) + geom_label(aes(label=codes), fontface="bold", color="white") + scale_fill_manual(name="Behaviour", values = coul)
#+ geom_point()
...
cowplot::ggsave("../../figs/phase1-cap.png", phase1.cca.plot, width=4, height=4, units=c("in"))
```
