...
phase1.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=sites, shape=behaviour, group=sites, label=sites))  + theme_bw() + theme(legend.position = "right") + guides(fill=guide_legend(title.position = "top")) + scale_color_manual(name="Region", values = coul) + geom_point(size=3) + scale_shape_manual(name="Behaviour",values=c(3,4,7,8,15,19,17,18,11,14)) 
