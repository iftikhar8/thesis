...
```{r}
reefs <- rownames(passive.conn.regions)
data.passive <- as_tibble(passive.conn.regions)
data.passive$sites <- reefs
...
community <- data.passive %>% full_join(data.diel) %>% full_join(data.orientate) %>% full_join(data.ovm) %>% full_join(data.diel.orientate) %>% full_join(data.ovm.orientate) %>% full_join(data.diel.ovm.orientate) %>% full_join(data.diel.ovm)
community$behaviour <- factor(community$behaviour)
community_matrix <- select(community, -behaviour, -sites)
...
MDS_xy$behaviour <- community$behaviour
nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=behaviour)) + geom_jitter(width=0.015,size=3) + theme_bw() + theme(legend.position = "right")  + scale_color_brewer(name = "", palette = "Paired") +stat_ellipse()

adonis(community_matrix ~ behaviour, community, perm=200)

mod <- with(community, betadisper(vegdist(community_matrix,"bray"), behaviour))
