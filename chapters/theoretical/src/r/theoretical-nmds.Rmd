...
Labels for the data
```{r}
model <- c("passive","dvm","ovm","ohs","dvm+ovm+ohs","ovm+ohs","dvm+ovm","dvm+ohs","pomacentridae","labridae","mullidae","scaridae","scorpaenidae","serranidae","synodontidae","stage","timestep","daily")
...
# nMDS 
reefs.nmds <- metaMDS(reefs.community_matrix, distance="bray", k=2, trymax=100)
...
regions.nmds <- metaMDS(regions.community_matrix,distance="bray", k=3, trymax=100)
...
regions.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=scenarios, shape=scenarios, label=model)) + theme_bw() + theme(legend.position = "bottom") + scale_color_brewer(name = "", palette = "Paired") + geom_label()
...
data <- as_tibble(passive.conn.regions/(passive.conn.regions+1))
data$sites <- reefs
data$behaviour <- rep(c("passive"))
data.diel <- as_tibble(diel.conn.regions/(passive.conn.regions+1))
data.diel$sites <- reefs
data.diel$behaviour <- rep(c("dvm"))
data.ovm <- as_tibble(ovm.conn.regions/(passive.conn.regions+1))
data.ovm$sites <- reefs
data.ovm$behaviour <- rep(c("ovm"))
data.orientate <- as_tibble(orientate.conn.regions/(passive.conn.regions +1))
data.orientate$sites <- reefs
data.orientate$behaviour <- rep(c("ohs"))
data.ovm.orientate <- as_tibble(ovm.orientate.conn.regions/(passive.conn.regions+1))
data.ovm.orientate$sites <- reefs
data.ovm.orientate$behaviour <- rep(c("ovm+ohs"))
data.diel.ovm.orientate <- as_tibble(diel.ovm.orientate.conn.regions/(passive.conn.regions+1))
data.diel.ovm.orientate$sites <- reefs
data.diel.ovm.orientate$behaviour <- rep(c("dvm+ovm+ohs"))
data.diel.orientate <- as_tibble(diel.orientate.conn.regions/(passive.conn.regions+1))
data.diel.orientate$sites <- reefs
data.diel.orientate$behaviour <- rep(c("dvm+ohs"))
data.diel.ovm <- as_tibble(diel.ovm.conn.regions/(passive.conn.regions+1))
...
phase1.nmds <- metaMDS(phase1.community_matrix,distance="bray", k=2, trymax=100)
#plot(nmds)
...
MDS_xy$sites <- factor(phase1.community$sites, levels=reefs)
phase1.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=sites, shape=behaviour, group=sites, label=sites))  + theme_bw() + theme(legend.position = "bottom") + guides(fill=guide_legend(title.position = "top")) + scale_color_manual(name="Region", values = coul) + geom_point(size=3) + scale_shape_manual(name="Behaviour",values=c(3,4,7,8,15,16,17,18)) 
#+geom_polygon(aes(fill=sites))
save_plot("../../figs/phase1-all-nmds.png",phase1.nmds.plot,base_aspect_ratio = 2.3)
...
phase2.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=sites, shape=fish, group=sites, label=sites))  + theme_bw() + theme(legend.position = "bottom") + guides(fill=guide_legend(title.position = "top")) + scale_color_manual(name="Region", values = coul) + geom_point(size=3) + scale_shape_manual(name="OVM Strategy",values=c(3,4,7,8,15,16,17,18)) 
#+geom_polygon(aes(fill=sites))
save_plot("../../figs/phase2-all-nmds.png",phase2.nmds.plot,base_aspect_ratio = 3)
...
phase3.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=sites, shape=methods, group=sites, label=sites))  + theme_bw() + theme(legend.position = "bottom") + guides(fill=guide_legend(title.position = "top")) + scale_color_manual(name="Region", values = coul) + geom_point(size=3) + scale_shape_manual(name="OVM Method",values=c(15,16,17)) 
#+geom_polygon(aes(fill=sites))
save_plot("../../figs/phase3-all-nmds.png",phase3.nmds.plot,base_aspect_ratio = 2.3)
...
all.nmds.plot <- ggplot(all.ca.data, aes(CA1, CA2, colour=scenario, group=scenario, label=model))  + theme_classic() + theme(legend.position = "right") + geom_point(size=3) + geom_text_repel()
...
metrics.plot1 <- ggplot(data=metrics, aes(model, diversity, fill=scenario),na.rm = TRUE) + stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") + coord_flip() + labs(x = "", y="Settlement diversity")+ theme(legend.position = "bottom") + scale_fill_discrete(name="") +guides(fill=guide_legend(nrow=2,byrow=TRUE))

metrics.plot2 <- ggplot(data=metrics, aes(model, richness, fill=scenario),na.rm = TRUE) + stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") + coord_flip()  + labs(x = "", y="Settlement richness")+ theme(legend.position = "bottom") + scale_fill_discrete(name="") +guides(fill=guide_legend(nrow=2,byrow=TRUE))

load(file="graph.rda")
connectance.data$model <- factor(connectance.data$model, levels=rev(model.levels))
metrics.plot3 <- ggplot(data=connectance.data, aes(model, connectance, colour=scenario),na.rm = TRUE) + geom_point() + coord_flip()  + labs(x = "", y="Connectance")+ theme(legend.position = "bottom") + scale_colour_discrete(name="") +guides(colour=guide_legend(nrow=2,byrow=TRUE))

metrics.grid <- plot_grid(metrics.plot2,metrics.plot1, metrics.plot3, labels="AUTO", align ="hv")
save_plot("../../figs/settle-metrics.png", metrics.grid, ncol=2, nrow=2)
```
