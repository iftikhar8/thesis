...
Labels for the data
```{r}
model <- c("passive","dvm","ovm","ohs","dvm+ovm+ohs","ovm+ohs","dvm+ovm","dvm+ohs","pomacentrid","labrid","mullid","scarid","scorpaenid","serranid","synodontid","stage","timestep","daily")
...
data.orientate <- as_tibble(orientate.conn.regions/(passive.conn.regions[,-18:-19]+1))
data.orientate$sites <- reefs
data.orientate$behaviour <- rep(c("ohs"))
data.ovm.orientate <- as_tibble(ovm.orientate.conn.regions/(passive.conn.regions[,-18]+1))
data.ovm.orientate$sites <- reefs
data.ovm.orientate$behaviour <- rep(c("ovm+ohs"))
data.diel.ovm.orientate <- as_tibble(diel.ovm.orientate.conn.regions/(passive.conn.regions[,-18:-19]+1))
data.diel.ovm.orientate$sites <- reefs
data.diel.ovm.orientate$behaviour <- rep(c("dvm+ovm+ohs"))
data.diel.orientate <- as_tibble(diel.orientate.conn.regions/(passive.conn.regions[,-18:-19]+1))
data.diel.orientate$sites <- reefs
data.diel.orientate$behaviour <- rep(c("dvm+ohs"))
data.diel.ovm <- as_tibble(diel.ovm.conn.regions/(passive.conn.regions[,-18:-19]+1))
...
phase1.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=behaviour, group=sites, label=sites))  + theme_bw() + theme(legend.position = "bottom")  + scale_color_brewer(name = "", palette = "Paired") + stat_ellipse(type="norm",linetype = 1) + geom_text()
...
```{r}
reefs <- c("Tweed","Byron","Ballina","Yamba","Solitary Is","Nambucca","Pt Macquarie","Crowdy Bay","Pt Stephens","Newcastle","Sydney","Wollongong","Jervis Bat","Ulladulla","Batemans","Merimbula","Eden")
data.labrid <- as_tibble(ovm.labrid.conn.regions/(ovm.conn.regions[,-18:-19]+1))
data.labrid$sites <- reefs
data.labrid$fish <- rep(c("labridae"))
data.mullid <- as_tibble(ovm.mullid.conn.regions[,-18]/(ovm.conn.regions[,-18:-19]+1))
data.mullid$sites <- reefs
data.mullid$fish <- rep(c("mullidae"))
data.pomacentrid <- as_tibble(ovm.conn.regions[,-18:-19]/(ovm.conn.regions[,-18:-19]+1))
data.pomacentrid$sites <- reefs
data.pomacentrid$fish <- rep(c("pomcentridae"))
data.scarid <- as_tibble(ovm.scarid.conn.regions/(ovm.conn.regions[,-18:-19]+1))
data.scarid$sites <- reefs
data.scarid$fish <- rep(c("scaridae"))
data.synodontid <- as_tibble(ovm.synodontid.conn.regions/(ovm.conn.regions[,-18:-19]+1))
data.synodontid$sites <- reefs
data.synodontid$fish <- rep(c("synodontidae"))
data.scorpaenid <- as_tibble(ovm.scorpaenid.conn.regions/(ovm.conn.regions[,-18:-19]+1))
data.scorpaenid$sites <- reefs
data.scorpaenid$fish <- rep(c("scorpaenidae"))
data.serranid <- as_tibble(ovm.serranid.conn.regions[,-18]/(ovm.conn.regions[,-18:-19]+1))
...
phase2.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=fish, group=sites, label=sites))  + theme_bw() + theme(legend.position = "bottom")  + scale_color_brewer(name = "", palette = "Paired") + stat_ellipse(type="norm",linetype = 1) + geom_text()
...
```{r}
reefs <- c("Tweed","Byron","Ballina","Yamba","Solitary Is","Nambucca","Pt Macquarie","Crowdy Bay","Pt Stephens","Newcastle","Sydney","Wollongong","Jervis Bat","Ulladulla","Batemans","Merimbula","Eden")
data.timestep <- as_tibble(ovm.timestep.conn.regions/(ovm.conn.regions[,-18:-19]+1))
data.timestep$sites <- reefs
data.timestep$methods <- rep(c("timestep"))
data.daily <- as_tibble(ovm.daily.conn.regions[,-18]/(ovm.conn.regions[,-18:-19]+1))
data.daily$sites <- reefs
data.daily$methods <- rep(c("daily"))
data.stage <- as_tibble(ovm.conn.regions[,-18:-19]/(ovm.conn.regions[,-18:-19]+1))
...
phase3.nmds.plot <- ggplot(MDS_xy, aes(MDS1, MDS2, colour=methods, group=sites, label=sites))  + theme_bw() + theme(legend.position = "bottom")  + scale_color_brewer(name = "", palette = "Paired") + stat_ellipse(type="norm",linetype = 1) + geom_label()
...
# shannons
phase1.reefs.shannon <- vegan::diversity(phase1.reefs.data.t)
...
# shannons
phase2.reefs.shannon <- vegan::diversity(phase2.reefs.data.t)
...
```{r}
grid <- plot_grid(phase1.plot,phase2.plot, phase3.plot, labels=c("a","b", "c"), align ="h")
