...
phase2.nmds.plot.norm <- ggplot(phase2.points.norm, aes(MDS1, MDS2, colour=fish, group=sites,fill=fish))  + theme_classic() + theme(legend.position = "right") + guides(fill=guide_legend(title.position = "top")) + geom_label(aes(label=codes), fontface="bold", color="white") + scale_fill_manual(name="OVM strategy", values = coul)
#+geom_polygon(aes(fill=sites))
...
phase3.cap.plot <- ggplot(phase3.cca, aes(CAP1, CAP2))  + theme_classic() + geom_label_repel(aes(label=methods),size=3) + geom_point() + labs(x="CAP1 (26.9% explained)",  y="CAP2 (13.9% explained)")

ggsave("../../figs/phase3-cap.png", phase3.cap.plot, width=6, height=4)
```
...
all.cap.plot <- ggplot(all.cca, aes(CAP1, CAP2, label=models, colour=scenario, shape=scenario))  + theme_classic() +geom_label_repel(show.legend = FALSE,size=3) + geom_point(size=2) + labs(x="CAP1 (23.4% explained)", y="CAP2 (12.6% explained)") + scale_colour_brewer(name="Scenario",palette = "Paired") + scale_shape(name="Scenario")
ggsave("../../figs/all-cap.png", all.cap.plot,width=7,height=4)
