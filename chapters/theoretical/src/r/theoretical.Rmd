...
p <- ggplot(ovm, aes(x=stage, y=depth, weight=probability, fill=stage)) +  geom_violin(adjust = .85, trim=FALSE)  + theme_few()  + labs(y="Depth (m)", x="Ontogenetic stage") + theme(legend.position = "none") + scale_y_reverse(limits=c(100,0)) + scale_fill_manual(values = pal)
...
pal <- canva_pal("Fresh and energetic")(4)
p <- ggplot(diel, aes(x=time, y=depth, weight=probability, fill=time)) +  geom_violin(adjust = .85, trim=FALSE)  + theme_few()  + labs(y="Depth (m)", x="Time of day") + theme(legend.position = "none") + scale_y_reverse(limits=c(100,0)) + scale_fill_manual(values = pal)
