...
from.to.plot <- ggplot(aes(y=from.mean,x=to.mean),data=from.to.mpa.data) + geom_point(aes(size=CV,color=mpa) ) + geom_text_repel(size=4, label=cv.from.names) + scale_color_manual(values = pal,name="Sanctuary zone") + labs(x="Settlement to", y="Settlement from") 
save_plot("../../figs/mpa.png", from.to.plot)
