...
CCA for years data
```{r}
years.data.matrix <- spread(years.data.mean, settle.region, value)
years.matrix <- as.data.frame(years.data.mean[-1])
colnames(years.matrix) <- c("Tweed","Byron","Ballina","Yamba","Solitary Is","Nambucca","Pt Macquarie","Crowdy Bay","Pt Stephens","Newcastle","Sydney","Wollongong","Jervis Bay","Ulladulla","Batemans Bay","Merimbula","Eden","Lord Howe Is","EM Reefs","Queensland","Victoria")
rownames(years.matrix) <- rev(years)
years.ca <- CA(years.matrix, graph=FALSE)
summary(years.ca)
fviz_screeplot(years.ca, addlabels=TRUE)
years.biplot <- fviz_ca_biplot(years.ca, map="colgreen", arrow=c(FALSE,TRUE), repel=TRUE, title = "") + theme_classic()
fviz_ca_row(years.ca, repel=TRUE)
fviz_ca_col(years.ca, repel=TRUE)
row <- get_ca_row(years.ca)
row$contrib
ggsave("../../figs/cca-years.png",years.biplot, width=7.5, height=5, units=c("in"))
```
...
sites.diversity <- ggplot(community, aes(sites, diversity)) + stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") + coord_flip() + theme_classic()
sites.richness <- ggplot(community, aes(sites, richness)) + stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") + coord_flip() + theme_classic()
years.diversity <- ggplot(community, aes(year, diversity)) + stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") + coord_flip() + theme_classic()
years.richness <- ggplot(community, aes(year, richness)) + stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") + coord_flip() + theme_classic()
p <- plot_grid(sites.diversity, sites.richness, years.diversity, years.richness)
