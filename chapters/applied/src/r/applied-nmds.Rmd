...
```{r}
regions.community_matrix <- regions.data %>% ungroup() %>% select(-source.region) %>% gather(year,value,2:9) %>% group_by(year) %>% mutate(row = 1:n()) %>% spread(settle.region, value)
#png('../../figs/regions-nmds.png')
regions.community_matrix[is.na(regions.community_matrix)] <- 0

#community_matrix.transposed[is.na(community_matrix.transposed)] <- 0
regions.nmds <- metaMDS(regions.community_matrix[,-1:-2],distance="bray", k=2)
...
MDS_xy$region <- regions.community_matrix$source.region
MDS_xy$year <- regions.community_matrix$year
