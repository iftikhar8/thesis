...
all.regions <- group_by(reefs.yearly, settle.region, year)
all.regions <- summarize(all.regions , count = sum(settlement/eggs))
ggplot(all.regions, aes(x=settle.region, y=count)) + geom_boxplot(aes(settle.region)) + coord_flip() +scale_x_discrete(limits = rev(levels(all.regions$settle.region)))
```

Settlement to all regions
```{r}

#reefs.pinnacle <- reefs.pinnacle %>% summarise(sum)
all.regions <- group_by(reefs.yearly, source.region, year)
all.regions <- summarize(all.regions , count = sum(settlement/eggs))
ggplot(all.regions, aes(x=source.region, y=count)) + geom_boxplot(aes(source.region)) + coord_flip() +scale_x_discrete(limits = rev(levels(all.regions$source.region))) + theme_bw()
```
