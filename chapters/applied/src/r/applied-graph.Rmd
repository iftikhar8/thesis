...
```{r}
regions.data.graph <- regions.data %>% ungroup() %>% gather(year,value,3:10) %>% group_by(source.region, settle.region) %>% summarise(weight = round(mean(value),0), cv = round((sd(value)/mean(value))*100,0)) %>% filter(settle.region != "VIC" & settle.region != "QLD")

blackcod.graph.directed <- graph_from_data_frame(regions.data.graph, directed=TRUE)
regions.data.graph <- ungroup(regions.data.graph)
mean(regions.data.graph$settlers)
regions.data.graph <- group_by(regions.data.graph, source.region, settle.region) %>% spread(settle.region, settlers, fill=0) %>% ungroup() %>% as.data.frame()
...
dev.off()
ggdendrogram(wc, rotate = FALSE, size = 2)

mean.cut.off <- mean(E(blackcod.graph.directed)$weight)
cv.cut.off <- mean(E(blackcod.graph.directed)$cv)
#graph.sp <- delete_edges(graph, E(graph)[weight < cut.off])
png(filename="../../figs/network-all-regions.png")
blackcod.graph.directed.minimal <- delete.edges(blackcod.graph.directed, E(blackcod.graph.directed)[cv > 50])
...
blackcod.graph.directed.minimal <- simplify(blackcod.graph.directed.minimal)
plot(wc,blackcod.graph.directed.minimal, edge.arrow.size=0.1, edge.curved=0.1, edge.width=E(blackcod.graph.directed.minimal)$weight/500, vertex.label.family="Helvetica", vertex.label.color="black", vertex.label.cex=1.5, vertex.label=region.names, vertex.label.dist=2, vertex.size=5, layout=layout_in_circle)
dev.off()

p1 <- ggdraw() + draw_image("../../figs/dendo-all-regions.png")
p2 <- ggdraw()+ draw_image("../../figs/network-all-regions.png")
grid <- plot_grid(p1,p2, labels="AUTO", align="h", nrow=1)
save_plot("../../figs/graph.png", grid, nrow=1, ncol=2, base_aspect_ratio = 1.1)
