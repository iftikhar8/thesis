...
ggplot(solitary.regions, aes(x=settle.region, y=count)) + geom_boxplot(aes(settle.region)) + coord_flip() +scale_x_discrete(limits = rev(levels(eliza.middle.regions$settle.region)))
...
```{r}
lord.howe <- filter(reefs.yearly, settle.region=="NSW18")
#reefs.pinnacle <- reefs.pinnacle %>% summarise(sum)
lord.howe.regions <- group_by(lord.howe, source.region, year)
lord.howe.regions <- summarize(lord.howe.regions , count = sum(settlement/eggs))
ggplot(lord.howe.regions, aes(x=source.region, y=count)) + geom_boxplot(aes(source.region)) + coord_flip() +scale_x_discrete(limits = rev(levels(lord.howe.regions$source.region))) + theme_bw()
