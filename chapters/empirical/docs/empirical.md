...
This study focuses on investigating the ontogenetic vertical migration of larval fish, which through connectivity modelling studies has been shown to be important for influencing dispersal patterns [@Paris:2007hm; @Huebert:2011ci]. In general, it appears that larval fish get deeper in the water column with ontogeny [@Irisson:2010um], yet is relatively unknown for many species as ontogenetic vertical patterns are not consistent, varying between species, within species and over spatial contexts [@Leis:2010jz], e.g. Pomacentridae larvae have been shown to migrate vertically downward with ontogeny [@Paris:2004te] or display no changes of depth by ontogeny [@Irisson:2010um].

To measure realised patterns of connectivity, more biophysical dispersal studies are including vertical migration [CITE examples]. One reason to not include vertical migration behaviour is that for many species is unknown. In New South Wales, not many studies have looked at the vertical migration of larval fish (CITe Grays papers). Therefore our study aims to measuring the vertical migration distributions of reef associated families off the coast of New South Wales will allow us to accurately parameterise biophysical connectivity models with ontogenetic vertical migration behaviour to produce realised connectivity patterns. 

-------------
I would start this paragraph by referencing the knowledge gap about OVM patterns of reef fish and the importance to understanding connectivity from dispersal modeling. Then you can say that this is particularly important in this region as there is very complex oceanography which drives these eddies. And in addition to the interaction between this dynamic oceanography and OVM being critical to subsequent dispersal patterns, there is also a large unknown about any effects of eddies on OVM patterns themselves.

I you can say anything about the consistency of OVM patterns across different oceanographic features…or even just studies…that might be support for the hypothesis I suggest you include below…which is that you don't expect eddies to affect the OVM pattern in general.
-----------------
