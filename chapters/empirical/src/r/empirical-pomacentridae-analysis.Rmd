...
Positive predictor
```{r}
ggplot(data=data.ontogeny, aes(y=conc, x=salinity)) + geom_point() + geom_smooth(method="glm" , method.args = list(family="pi")) +labs(x="Salinity", y="Concentration", title="pomacentridae") + theme_classic()
...
```{r}
ggplot(data=data.length, aes(y=total_length, x=chlorophyll_fluorescence)) + geom_point() + geom_smooth(method=lm) + labs(x="Chlorophyll", y="Length (mm)", title="pomacentridae") + theme_classic()
...
plot(model.length.full) model.ontogeny.featuredepthstage.olre
```
