...
```{r}
ggplot(data=data.length, aes(y=total_length, x=temperature))+ geom_smooth(method=lm) + geom_point() + labs(x="Temperature", y="Length (mm)", title="serranidae") + theme_classic()
...
```{r}
ggplot(data=data.length, aes(y=total_length, x=salinity)) + geom_smooth(method=lm) + geom_point() + labs(x="Salinity", y="Length (mm)", title="serranidae") + theme_classic()
...
```{r}
ggplot(data=data.length, aes(y=total_length, x=chlorophyll_fluorescence)) + geom_point() + geom_smooth(method=lm) + labs(x="Chlorophyll", y="Length (mm)", title="serranidae") + theme_classic()
...
```{r}
ggplot(data=data.length, aes(y=total_length, x=dissolved_oxygen)) +geom_smooth(method=lm)+ geom_point() + labs(x="Dissolved Oxygen", y="Length (mm)", title="serranidae") + theme_classic()
...
So they were bigger in the eddy
```{r}
summary(model.length.full <- glm(total_length ~ depth + temperature + dissolved_oxygen + salinity + chlorophyll_fluorescence, family = gaussian(link="log"), data=data.length))
