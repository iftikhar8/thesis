...
ggplot(data=data.scarid.ontogeny, aes(y=conc, x=depth, group=stage, fill=stage)) + stat_summary_bin(fun.y="mean", geom="bar", position=position_dodge(width=0.90)) + stat_summary(fun.data = mean_se, geom = "errorbar", aes(width=0.15),position=position_dodge(width=0.90)) + labs(x="Depth", y="Concentration", title="scaridae") + theme_classic() +coord_flip() +  scale_x_discrete(limits = rev(levels(data.scarid.ontogeny$depth))) +facet_wrap(~feature)
...
data.ontogeny$stage.prefle <- data.ontogeny$stage %>% fct_collapse(PREFLE = c("PRE","FLE"))
summary(model.ontogeny.featuredepthstage.zinb <- zeroinfl(count ~ offset(log(volume)) + feature*depth*stage.prefle,  dist="negbin", link="logit", data=data.ontogeny))
...
So they were bigger in the eddy
```{r}
data.length$temperature.scale <- scale(data.length$temperature)
data.length$salinity.scale <- scale(data.length$salinity)
...
pairs(lsmeans(model.length.full, ~depth*feature))
