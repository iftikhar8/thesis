...
```{r}
ggplot(data=data.ontogeny, aes(y=conc, x=stage, fill="red")) + stat_summary_bin(fun.y="mean", geom="bar") + stat_summary(fun.data = mean_se, geom = "errorbar", aes(width=0.25)) + labs(x="Stage", y="Concentration", title="mullidae") + theme_classic() + guides(fill="none")
...
```{r}
data.ontogeny$stage <- factor(data.ontogeny$stage, levels = c("POS","FLE","PRE"))
...
So they were bigger in the eddy
```{r}
summary(model.length.full <- glm(total_length ~ feature*depth + temperature + chlorophyll_fluorescence + salinity, family = gaussian(link="log"), data=data.length))
