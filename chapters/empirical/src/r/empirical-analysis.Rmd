...
data.ontogeny.interaction_ds <-  dplyr::summarize(group_by(na.omit(data.ontogeny), family, depth,stage), conc.mean = mean(conc), conc.se = sd(conc)/n())
data.ontogeny.interaction_ds.abund <-  dplyr::summarize(group_by(na.omit(data.ontogeny), family, depth,stage), conc.mean = mean(count), conc.se = sd(count)/n())
data.ontogeny.interaction_fds <-  dplyr::summarize(group_by(na.omit(data.ontogeny), feature, family, depth,stage), conc.mean = mean(conc), conc.se = sd(conc)/n())
data.ontogeny.depth <-  dplyr::summarize(group_by(na.omit(data.ontogeny), family, depth), conc.mean = mean(conc), conc.se = sd(conc)/n())
data.ontogeny.stage <-  dplyr::summarize(group_by(na.omit(data.ontogeny), family, stage), conc.mean = mean(conc), conc.se = sd(conc)/n())
data.ontogeny.feature <-  dplyr::summarize(group_by(na.omit(data.ontogeny), family, feature), conc.mean = mean(conc), conc.se = sd(conc)/n())
...
#data.ontogeny$depth.r <- factor(data.ontogeny$depth, levels=c("75","25","0"))
data.ontogeny <- mutate(data.ontogeny, conc.t = conc*1000)
data.ontogeny$family.label.feature <- factor(data.ontogeny$family,labels=c("Labridae*","Mullidae*","Pomacentridae","Scaridae*","Scorpaenidae","Serranidae","Synodontidae"))
dat_text <- data.frame(
...
abundance.feature.plot <- ggplot(aes(y=conc.t, x=feature, fill=feature), data=data.ontogeny) +stat_summary_bin(fun.y="mean", geom="bar") + stat_summary(fun.data = "mean_cl_boot", geom = "linerange",show.legend = FALSE) + facet_wrap(~family.label.feature, ncol=4, scales = "free") + labs(x="", y=expression("Concentration per 1000"~m^{3})) + scale_fill_manual(values=pal,name="Feature")  + theme_cowplot(font_size=12,font_family = "sans") +theme(legend.position = c(0.95, 0.15), legend.justification = c(1, 0), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + scale_y_continuous(
  labels = scales::number_format(accuracy = 0.1))
cowplot::ggsave("../../figs/feature-abundance.png", plot=abundance.feature.plot, width=7, height=4, units=c("in"))
```
...
data.ontogeny.weighted.total <- data.ontogeny %>% filter(conc != 'NA') %>% mutate(depth.w = if_else(depth==0,0,if_else(depth==25,28,75.5))) %>% group_by(family.label, stage.label, depth.w) %>% summarise(total= sum(count),conc.sum = sum(conc)) %>% mutate(prop = conc.sum / (sum(conc.sum)+0.000001)) %>% mutate(depth.z = depth.w*prop) %>% group_by(family.label, stage.label) %>% summarise(depth.weighted = sum(depth.z))

data.ontogeny.weighted.totalcount <- data.ontogeny %>% mutate(depth.w = if_else(depth==0,0,if_else(depth==25,28,75.5))) %>% group_by(family.label, stage.label, depth.w) %>% summarise(total= sum(count)) %>% mutate(prop = total / (sum(total)+0.000001)) %>% mutate(depth.z = depth.w*prop) %>% group_by(family.label, stage.label) %>% summarise(depth.weighted = sum(depth.z))

data.ontogeny.weighted.prop <- data.ontogeny %>% mutate(depth.w = if_else(depth==0,0,if_else(depth==25,28,75.5))) %>% group_by(family.label, stage.label, depth.w) %>% summarise(total= sum(count)) %>% mutate(prop = total / (sum(total)+0.000001))
...
# count.graph <- ggplot(aes(x=stage.label,y=depth, weight=prop, fill=stage.label), data=data.ontogeny.prop) + geom_count() + facet_wrap(~ family.label, ncol=4) +theme_cowplot(font_size=12,font_family = "sans") +theme(legend.position = c(0.95, 0.15), legend.justification = c(1, 0), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + labs(y="Depth (m)", x=element_blank())  + scale_fill_manual(values=pal,name="Stage") 
# 
# + geom_line(data=data.ontogeny.weighted.total, inherit.aes = FALSE, aes(x=stage.label,y=depth.scale, group=family.label)) + geom_point(data=data.ontogeny.weighted.total, inherit.aes = FALSE, aes(x=stage.label,y=depth.scale)) #g
# 
...
## Model selection
```{r}
model.ontogeny.labrid.nb.select <- glm.nb(count ~ feature*depth*stage + scale(temperature) + offset(log(volume)),data=data.labrid.ontogeny)

model.length.labrid.gamma.select <- glm(standard_length ~ feature + depth + temperature + chlorophyll_fluorescence, family = Gamma, data=filter(data.length, family == "labridae"))

stepAIC(model.ontogeny.labrid.nb.select)
stepAIC(model.length.labrid.gamma.select)
...
cld(glht(data.ontogeny.labrid.posthoc.ovm.stage, alpha=0.05, Letters=letters, type="response",adjust="Tukey"))
...
# Model selection
```{r}
model.ontogeny.mullid.nb.select <- glm.nb(count ~ offset(log(volume)) + feature*depth*stage + scale(chlorophyll_fluorescence), data=data.mullid.ontogeny)
stepAIC(model.ontogeny.mullid.nb.select)

model.length.mullid.gamma.select <- glm(standard_length ~ feature*depth + temperature + dissolved_oxygen, family = Gamma(link = "inverse"), data=data.mullid.length)
stepAIC(model.length.mullid.gamma.select)
...
```{r}
model.ontogeny.pomacentrid.nb.select <- glm.nb(count ~ offset(log(volume)) + feature * depth * stage.posfle + scale(temperature) + scale(chlorophyll_fluorescence), data=data.pomacentrid.ontogeny)
model.length.pomacentrid.gamma.select <- glm(standard_length ~ feature * depth + temperature + salinity + dissolved_oxygen, family = Gamma, data=data.pomacentrid.length)

stepAIC(model.ontogeny.pomacentrid.nb.select)
stepAIC(model.length.pomacentrid.gamma.select)
```
...
```{r}
model.ontogeny.scarid.nb.select <- glm.nb(count ~ offset(log(volume)) + feature + feature*depth + feature*stage +  depth*stage + scale(temperature) + scale(dissolved_oxygen) +scale(salinity), data=data.scarid.ontogeny)

model.length.scarid.gamma.select <- glm(standard_length ~ feature*depth + temperature + salinity, family=Gamma, data=data.scarid.length)

stepAIC(model.ontogeny.scarid.nb.select)
stepAIC(model.length.scarid.gamma.select)
```
...
summary(model.ontogeny.scarid.nb <- glm.nb(count ~ offset(log(volume)) + feature + depth + feature*depth + stage + scale(temperature) + scale(dissolved_oxygen) +scale(salinity), data=data.scarid.ontogeny))
...
## Model selection
```{r}
model.ontogeny.scorpaenid.nb.select <- glm.nb(count ~ offset(log(volume)) + feature*stage*depth + scale(temperature), data=data.scorpaenid.ontogeny)
model.length.scorpaenid.gamma.select <- glm(standard_length ~ feature*depth + salinity, family = Gamma, data=data.scorpaenid.length)

stepAIC(model.ontogeny.scorpaenid.nb.select)
stepAIC(model.length.scorpaenid.gamma.select)
...
```{r}
model.ontogeny.serranid.nb.select <- glm.nb(count ~ offset(log(volume)) + depth*stage*feature+ scale(dissolved_oxygen), data=data.serranid.ontogeny)

model.length.serranid.gamma.select <- glm(standard_length ~ feature * depth + temperature, family = Gamma(link="log"), data=data.serranid.length)

stepAIC(model.ontogeny.serranid.nb.select)
stepAIC(model.length.serranid.gamma.select)
...
```{r}
model.ontogeny.synodontid.nb.select <- glm.nb(count ~ offset(log(volume)) + feature*depth*stage +scale(temperature) + scale(dissolved_oxygen), data=data.synodontid.ontogeny)

model.length.synodontid.gamma.select <- glm(standard_length ~ feature * depth + dissolved_oxygen, family = Gamma, data=data.synodontid.length)

stepAIC(model.ontogeny.synodontid.nb.select)
stepAIC(model.length.synodontid.gamma.select)
```
