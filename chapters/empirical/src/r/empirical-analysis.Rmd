...
p <- ggplot(aes(y=standard_length, x=depth.r), data=filter(data.length, standard_length < 30)) + geom_boxplot()+ facet_wrap(~family.label,ncol=4, scales = "free") + coord_flip() +labs(x="Depth (m)", y=expression("Length (mm)")) + theme_few()
...
plot <- ggplot(aes(x=stage.label,y=depth.weighted, colour=family.label, group=family.label),data=data.ontogeny.weighted.total)  + scale_y_reverse() + geom_line(size=1.5, alpha=0.75) + scale_colour_brewer(name="Family", palette = "Set2") + labs(x="Stage",y="Depth (m)")  + theme_few()

plot2 <- ggplot(aes(x=stage.label,y=depth.weighted, colour=family.label, group=family.label),data=data.ontogeny.weighted.totalcount)  + scale_y_reverse() + geom_line(size=1.5, alpha=0.75) + scale_colour_brewer(name="Family", palette = "Set2") + labs(x="Stage",y="Depth (m)") + theme_few()
