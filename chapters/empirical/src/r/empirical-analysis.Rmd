...
```{r}
ggplot(aes(y=standard_length, x=temperature), data=data.length)+ geom_point()+theme_bw() +labs(x="Temperature (C)", y=expression("Standard length (mm)")) + facet_wrap(~family, scales="free",ncol=4) + stat_smooth(method="lm", formula = y ~ x, se=T, level=0.95)
