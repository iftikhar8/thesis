...
data.ontogeny.weighted.total <- data.ontogeny %>% filter(conc != 'NA') %>% mutate(depth.w = if_else(depth==0,5,if_else(depth==25,28,75.5))) %>% group_by(family.label, stage.label, depth.w) %>% summarise(count= sum(count),conc.sum = sum(conc)) %>% mutate(prop = count / (sum(count)+0.000001)) %>% mutate(depth.z = depth.w*prop) %>% group_by(family.label, stage.label) %>% summarise(depth.weighted = sum(depth.z))
...
data.ontogeny.weighted.station <- data.ontogeny %>% filter(conc != 'NA') %>% mutate(depth.w = if_else(depth==0,5,if_else(depth==25,28,75.5))) %>% group_by(site, family, stage, depth.w) %>% summarise(count= sum(count),conc.sum = sum(conc)) %>% mutate(prop = count / (sum(count)+0.000001)) %>% mutate(depth.z = depth.w*prop) %>% group_by(site, family, stage) %>% summarise(depth.weighted = sum(depth.z))
...
plot <- ggplot(aes(x=stage.label,y=depth.scale, weight=prop, fill=stage.label), data=data.ontogeny.prop)  + scale_y_reverse(limits=c(100,0)) + geom_violin(adjust = .85, trim=FALSE) + facet_wrap(~ family.label, ncol=4) +theme(legend.position = c(0.95, 0.15), legend.justification = c(1, 0), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + labs(y="Depth (m)", x="")  + scale_fill_viridis(name="Stage", discrete = TRUE)
ggsave("../../figs/stages-by-family.png", plot, width=8, height=6)
