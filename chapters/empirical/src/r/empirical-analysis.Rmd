...
```{r}
Anova(model.length.labrid, type="II", test="Wald")

model.length.labrid.posthoc <- lsmeans(model.length.labrid, pairwise ~ depth, adjust="Tukey", type="response")

model.length.labrid.posthoc

cld(model.length.labrid.posthoc, alpha=0.05, Letters=letters, type="response",adjust="Tukey")

lsmip(model.length.labrid, ~ depth, type="response")
```
...
model.length.mullid.posthoc <- lsmeans(model.length.mullid.gamma, pairwise ~ depth, adjust="Tukey", type="response")

model.length.mullid.posthoc

cld(model.length.mullid.posthoc, alpha=0.05, Letters=letters, type="response",adjust="Tukey")
...
model.length.pomacentrid.posthoc <- lsmeans(model.length.pomacentrid.gamma, pairwise ~ depth, adjust="Tukey", type="response")
