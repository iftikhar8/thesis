...
## Ontogeny
```{r}
summary(model.ontogeny.lab.nb <- glm.nb(count ~ offset(log(volume)) + feature*depth*stage + scale(temperature) + scale(chlorophyll_fluorescence), data=filter(data.ontogeny, family=="labridae")))
summary(model.ontogeny.lab.p <- glm(count ~ offset(log(volume)) + feature*stage + depth*stage + scale(temperature) + scale(chlorophyll_fluorescence),  family=poisson, data=filter(data.ontogeny, family=="labridae")))
summary(model.ontogeny.lab.zinb <- zeroinfl(count ~ offset(log(volume)) + feature + depth*stage + scale(temperature) + scale(chlorophyll_fluorescence),  dist="negbin", link="logit", data=filter(data.ontogeny, family=="labridae")))
summary(model.ontogeny.lab.zip <- zeroinfl(count ~ offset(log(volume)) + feature*depth*stage + scale(temperature) + scale(chlorophyll_fluorescence), data=filter(data.ontogeny, family=="labridae")))

p.resids <- simulateResiduals(model.ontogeny.lab.p)
dispersiontest(model.ontogeny.lab.p)
testZeroInflation(p.resids)
nb.resids <- simulateResiduals(model.ontogeny.lab.nb)
testOverdispersion(nb.resids)
testZeroInflation(nb.resids)
vuong(model.ontogeny.lab.nb,model.ontogeny.lab.zinb)
```

## Length
```{r}
summary(model.length.full <- glm(total_length ~ feature*depth + temperature + chlorophyll_fluorescence, family = gaussian(link="log"), data=filter(data.ontogeny, family=="labridae")))
```
