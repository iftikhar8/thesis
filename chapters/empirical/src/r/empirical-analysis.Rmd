...
dat_text <- data.frame(
  family = c("labridae","mullidae","pomacentridae","scorpaenidae","serranidae"),
  x = c(3.15, 3.15, 3.15, 3.15, 3.15),
  y = c(12,12,12,12,12)
)

p <- ggplot(aes(y=standard_length, x=depth.r, group=1), data=data.length)+stat_summary_bin(fun.y="mean", geom="point") + stat_summary(fun.data = "mean_cl_boot", geom = "linerange",show.legend = FALSE) +stat_summary(fun.y="mean", geom="line")+ facet_wrap(~family,ncol=4) + theme_bw() + coord_flip() +labs(x="Depth (m)", y=expression("Length (mm)"))
p <- p + geom_text(
  data    = dat_text,
...
p<- ggplot(aes(y=standard_length, x=feature), data=data.length) +stat_summary_bin(fun.y="mean", geom="bar") + stat_summary(fun.data = "mean_cl_boot", geom = "linerange",show.legend = FALSE) + facet_wrap(~family, ncol=4) + theme_bw() + labs(x="Feature", y=expression("Length (mm)"))
p <- p + geom_text(
  data    = dat_text,
  mapping = aes(x = x, y = y, label = "*"),
  size = 8,
...
ggsave("../../figs/length-family-feature.png", plot=p, width=6.46, height=3.93, units=c("in"))
```
...
p <- ggplot(aes(y=conc, x=feature), data=data.ontogeny) +stat_summary_bin(fun.y="mean", geom="bar") + stat_summary(fun.data = "mean_cl_boot", geom = "linerange",show.legend = FALSE) + facet_wrap(~family, ncol=4) + theme_bw() + labs(x="Feature", y=expression("Concentration (mm)"))
p <- p + geom_text(
  data    = dat_text,
  mapping = aes(x = x, y = y, label = "*"),
  size = 8,
...
ggsave("../../figs/feature-abundance.png", plot=p, width=6.46, height=3.93, units=c("in"))
```
