...
So they were bigger in the eddy
```{r}
ggplot(data=data.length, aes(y=total_length, x=depth, fill="red")) + facet_grid(~feature)  + stat_summary_bin(fun.y="mean", geom="bar") + stat_summary(fun.data = mean_se, geom = "errorbar", aes(width=0.25)) + labs(x="Stage", y="Length (mm)", title="Labridae") + theme_classic() + guides(fill="none")
