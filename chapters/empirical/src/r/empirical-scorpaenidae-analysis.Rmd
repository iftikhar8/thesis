...
model.ontogeny.full.olre <- glmer(count ~ offset(log(volume)) + depth*stage + temperature.scale + salinity.scale + do.scale + chloro.scale + (1|obs), family=poisson(link="log"), data=data.ontogeny, nAGQ=10, glmerControl(optimizer="bobyqa"))
...
```{r}
ggplot(data=data.length, aes(y=total_length, x=temperature))+ geom_smooth(method=lm) + geom_point() + labs(x="Temperature", y="Length (mm)", title="scorpaenidae") + theme_classic()
...
```{r}
ggplot(data=data.length, aes(y=total_length, x=dissolved_oxygen)) +geom_smooth(method=lm)+ geom_point() + labs(x="Dissolved Oxygen", y="Length (mm)", title="scorpaenidae") + theme_classic()
...
So they were bigger in the eddy
```{r}
summary(model.length.full <- glm(total_length ~ depth + temperature + dissolved_oxygen, family = gaussian(link="log"), data=data.length))
