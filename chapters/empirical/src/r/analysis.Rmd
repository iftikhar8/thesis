...
data.conc <- data.gather %>% left_join(data.tow) %>% mutate(concentration = count/volume * 250)
...
Anova(model.conc.labrid, type = "III")
```
...
Anova(model.conc.mullid, type = "III")
```
...
Anova(model.conc.scarid, type = "III")
```
...
Anova(model.conc.scorp, type = "III")
```
...
Anova(model.conc.serr, type = "III")
```
