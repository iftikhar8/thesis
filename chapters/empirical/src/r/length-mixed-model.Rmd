...
```{r, include=FALSE}
setwd("~/Documents/PhD/Thesis/chapters/empirical/src/r")
require(tidyverse)
require(car)
require(MASS)
require(lmerTest)
...
## Model
```{r}
labrid.lme <- lmer(total_length ~ feature * depth + (1 | location) + (1 | feature : station), data=labrid.data)
summary(labrid.lme)
```
...
lsmip(synodontid.lme, ~ depth)
lsmip(synodontid.lme, ~ feature)
```
...
## Model
```{r}
serranid.glmm <- glmer(total_length ~ feature * depth + (1 | location) + (1 | feature : station), family=gaussian(link="log"), data=serranid.data)
