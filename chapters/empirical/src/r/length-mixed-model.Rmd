...
## Model
```{r}
labrid.lm <- lm(length_log ~ feature * depth + temperature + salinity + chlorophyll_fluorescence,  data=labrid.data)
summary(labrid.lm)
```
...
## Model
```{r}
mullid.glm <- glm(total_length ~ feature * depth + temperature + salinity + chlorophyll_fluorescence, data=mullid.data)
summary(mullid.glm)
overdispersed(mullid.glm)
```
