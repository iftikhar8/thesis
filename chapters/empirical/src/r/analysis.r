...
data.sum <- summarize(group_by(data, site, location, feature, net, depth, family, stage), count=n(),length_mean=mean(total_length),length_sd=sd(total_length), length_se=length_sd/n())
...
ggsave("families_count.png",path="../../figs/", width=5, height=4, dpi=100)

ggplot(data,aes(family,fill=stage)) + geom_bar(position="dodge") + theme(axis.text.x = element_text(angle=45,vjust=1,hjust=1))
