...
data.conc <- mutate(data.conc, presence = ifelse(conc > 0, 1 , 0))
```
...
# Distribution
```{r}
qqp(mullid.data.real$conc, "lnorm")
```

## Model
```{r}
mullid.glmm.nonzero <- glmer(conc ~ feature * depth * stage + (1 | location) + (1 | feature : station), family=gaussian(link="log"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)), data=mullid.data.nonzero)
mullid.glmm.bernoulli <- glmer(presence ~ feature * depth * stage + (1 | location) + (1 | feature : station), family=binomial(link="logit"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)), data=mullid.data)
summary(mullid.glmm.nonzero)
summary(mullid.glmm.bernoulli)
```
...
qqp(pomacentrid.data$conc_log, "norm")
```

## Model
```{r}
pomacentrid.glmm.nonzero <- glmer(conc ~ feature + depth + stage + (1 | location) + (1 | feature : station), family=gaussian(link="log"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)), data=pomacentrid.data.nonzero)
pomacentrid.glmm.bernoulli <- glmer(presence ~ feature * depth * stage + (1 | location) + (1 | feature : station), family=binomial(link="logit"), control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)), data=pomacentrid.data)
```
