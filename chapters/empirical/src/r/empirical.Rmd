...
coast <- get_map(c(lon=154.5, lat=-30.0), zoom = 7, maptype=c("hybrid"), color=c("bw"), crop = TRUE)
sites <- dplyr::select(data.tow, site, feature, latitude, longitude) %>% distinct()
australia <- ggmap(coast)
australia + geom_point(aes(x=longitude, y=latitude, group=site, colour=feature, shape=feature),size=1.5, data=sites)
