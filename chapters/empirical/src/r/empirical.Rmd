...
data.measure <- filter(data.measure, family != "bothid" & family != "triglid" & family != "unknown" & family != "pseudochromidae")
...
map <- openmap(c(lat= -26.5, lon= 151.5), c(lat= -33.5, lon= 155.5), type="esri-topo",minNumTiles=12L)
mapLatLon <- openproj(map)
g <- autoplot(mapLatLon)
pal <- canva_pal("Fresh and energetic")(4)
sites <- dplyr::select(data.tow, site, feature, latitude, longitude) %>% distinct()
#australia <- ggmap(coast)
map <- g + geom_point(aes(x=longitude, y=latitude, group=site, colour=feature, shape=feature), size=2, data=sites) + theme(legend.position = "top", legend.text = element_text(size=12)) + labs(x="Longitude", y="Latitude") + scale_color_manual(values = pal, labels=c("Coastal","Eddy"),name="Oceanic feature") + scale_shape_manual(values = c(17,19), labels=c("Coastal","Eddy"),name="Oceanic feature")
...
flexion <- ggdraw() + draw_image("../../figs/type_specimens/labrid-x-fle.png", scale=0.9)
preflexion <- ggdraw() + draw_image("../../figs/type_specimens/labrid-pre.png", scale=0.9)
postflexion <- ggdraw() + draw_image("../../figs/type_specimens/labrid-y-pos.png", scale=0.9)
...
summarize(group_by(data.measure,family,stage), avg_length=mean(total_length), sd_length=sd(total_length))
