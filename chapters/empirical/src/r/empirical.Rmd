...
coast <- get_map(c(lon=154.25, lat=-30.5), zoom = 7, maptype=c("hybrid"))
sites <- dplyr::select(data.tow, site, feature, latitude, longitude) %>% distinct()
australia <- ggmap(coast)
map <- australia + geom_point(aes(x=longitude, y=latitude, group=site, colour=feature, shape=feature), size=2.5, data=sites) + theme(legend.position = "top")
...
flexion <- ggdraw() + draw_image("../../figs/type_specimens/labrid_i_fle.png", scale=0.9)
preflexion <- ggdraw() + draw_image("../../figs/type_specimens/labrid_b_pre.png", scale=0.9)
postflexion <- ggdraw() + draw_image("../../figs/type_specimens/labrid_b_pos.png", scale=0.9)
