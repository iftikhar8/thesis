...
data.all %>% select(PLD_fixed,Settlement_competency_type_start) %>% na.omit
ggplot(data.all,aes(PLD_fixed,Settlement_competency_type_start)) + geom_point()
```
####Comparisons
```{r}
...
lmSR <- lm(window~Self_recruitment_mean,data.window)
summary(lmSR)
...
lmSS <- lm(window~Settlement_success_mean,data.window)
summary(lmSS)
...
lmLR <- lm(window~Local_retention_mean,data.window)
summary(lmLR)
