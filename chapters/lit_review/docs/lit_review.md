...
The second category focused on the inputs used to parametrise the physical model. Assessing the physical model included details such as the oceanographic model used, the particle tracking software, diffusion methods and the model time-step. 

The third category considered the input parameters of the biological model, recording the traits such as the pelagic larval duration and spawning times and the behaviours implemented, e.g. mortality, vertical migration and growth. 
